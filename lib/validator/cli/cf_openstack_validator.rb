...
module Validator::Cli
  class CfOpenstackValidator
    ...
    def install_cpi_release_from_config
      cpi_release_path = File.join(@context.working_dir, 'bosh-huaweicloud-cpi-release.tgz')
    ...
    def generate_cpi_config
      ...
      puts "CPI will use the following configuration: \n#{JSON.pretty_generate(Validator::Redactor.redact(cpi_config_content, 'cloud.properties.huaweicloud.api_key'))}"
    ...
    def render_cpi_executable
      cpi_erb = File.read(File.join(@context.extracted_cpi_release_dir, 'jobs', 'huaweicloud_cpi', 'templates', 'cpi.erb'))
